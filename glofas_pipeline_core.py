...
class BaseGlofasConfig:
    ...
    def __init__(self):
        ...
        # RP thresholds and the JRC cache both default to following DATA_PIPELINE_DB,
        # same as the per-cycle discharge/extent output: BLOB means only Blob,
        # SNOWFLAKE means only Snowflake, LOCAL means only local, with no implicit
        # cross-mode dependency. Each can still be overridden independently (e.g. Blob
        # for the per-cycle output but Snowflake for these static reference files, a
        # legitimate mixed-mode config) by setting GLOFAS_THRESHOLD_SOURCE/GLOFAS_JRC_SOURCE
        # explicitly. Local default lives under glofas_data_dir (not a separate top-level
        # dir) so local storage has one root, mirroring the Snowflake stage's own single
        # glofas/ root (thresholds_cache/, jrc_extent_cache/, and {date}/ all under it).
        self.glofas_threshold_source = os.getenv('GLOFAS_THRESHOLD_SOURCE', self.data_pipeline_db).lower()
        self.glofas_threshold_local_dir = os.getenv('GLOFAS_THRESHOLD_LOCAL_DIR',
                                                     f'{self.glofas_data_dir}/thresholds_cache')

        # JRC extent-masking cache: same "follows DATA_PIPELINE_DB by default, overridable
        # independently" reasoning as the RP thresholds above. Populated by
        # setup_jrc_extents.py (one-time, direct JRC download, run out-of-band never by
        # the recurring pipeline itself, see glofas_extent_masking.py's docstring).
        self.glofas_jrc_source = os.getenv('GLOFAS_JRC_SOURCE', self.data_pipeline_db).lower()
    ...
    def needs_blob_creds(self) -> bool:
        """True if at least one of these paths actually reads/writes Blob:
        the per-cycle discharge/extent output (data_pipeline_db=='BLOB'), the
        RP threshold file, or (extent enabled) the JRC cache. Mirrors
        _needs_stage_operations()'s role for Snowflake: BLOB, SNOWFLAKE, and
        LOCAL are genuinely independent per-field choices, so this must check
        each field's own literal value, not assume BLOB only ever means
        data_pipeline_db=='BLOB'."""
        return (self.data_pipeline_db == 'BLOB'
                or self.glofas_threshold_source == 'blob'
                or (self.glofas_extent_enabled and self.glofas_jrc_source == 'blob'))
    ...
    def validate(self) -> bool:
        ...
        # Blob credentials checked unconditionally here, before any Snowflake-
        # related early return below: a genuinely all-Blob config (BLOB +
        # threshold_source=blob + jrc_source=blob) needs zero Snowflake creds
        # and must not skip this check as a side effect of that.
        ...
        # data_pipeline_db=='SNOWFLAKE' case. Deliberately NOT gated on
        # data_pipeline_db != 'BLOB' (an earlier version of this check was, which
        # was itself a bug: data_pipeline_db=='BLOB' with threshold_source or
        # jrc_source still 'snowflake', a legitimate mixed-mode config, would
        # have silently skipped this requirement and failed later, mid-run,
        # instead of at startup). A GLOFAS_MODE=submit-only config with
        # everything local/blob correctly does NOT require a stage name, since
        # _needs_stage_operations() would be False for it, this whole block
        # never fires.
...
def _glofas_already_downloaded(config: BaseGlofasConfig, forecast_date: datetime,
    ...
    remote_path = f'glofas/{candidate_str}/river_{candidate_str}.zarr.zip'
    if snowflake_conn and config.snowflake_stage_name:
        if stage_file_exists(config.snowflake_stage_name, remote_path, snowflake_conn):
            return forecast_date
    # Blob check, same purpose as the Snowflake-stage check above: without this,
    # a BLOB-mode deployment with no local copy retained (CLEANUP_AFTER_LOAD=true)
    # and no Snowflake stage in use could never detect an already-uploaded file,
    # so a late-running submit trigger would fire a fresh, wasted, billable pair
    # of CDS requests every time. Same remote_path convention already used by the
    # real upload call in glofas_downloader.py (Snowflake stage and Blob share the
    # identical relative path).
    if config.blob_account_url and config.blob_sas_token and config.blob_container:
        if blob_file_exists(config.blob_account_url, config.blob_sas_token,
                             config.blob_container, remote_path):
            return forecast_date
    return None
...
def run_glofas_pipeline(config: BaseGlofasConfig, snowflake_conn=None,
    ...
    result = download_glofas_forecast(
        ...
        blob_sas_token=config.blob_sas_token,
